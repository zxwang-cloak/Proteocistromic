#!/usr/bin/env python3
"""Characterize motif spacing distributions for TF pairs using merged co-bound peaks."""
...
if __name__ == "__main__":
  ...
  #  with open(os.path.dirname(__file__)+'/home/zxwang/software/spacing_pipeline-main/scripts/simul_random_null.json', 'r') as JSON: 
    ...
    plt.ylabel('normalized # binding site')
    plt.xlabel('Spacing (bp)')
    plt.savefig(path+'/spacing_files/'+'_'.join([tf1, tf2])+'_spacingDistribution.pdf', format='pdf')
    plt.close()
